...
def build_facility_report(arboretum):
 ...
 +-++-++-++-++-++-++-++-++-++-++-++-++-++-++-++-++-+\n"""
    ...
    # biomes = ["coastlines", "forests", "grasslands", "mountains", "rivers", "swamps", "grazingFields"]
    # biomes = ["grazingFields"]
    ...
    animalFacilityTypes = ["grazingFields", "chickenHouses", "duckHouses"]
    for facilityType in animalFacilityTypes:
        for facility in arboretum.biomes[facilityType]:
            # check if there are entries in the plant or animal dictionaries
            print(facility)
            if (
                len(facility.animals.keys()) > 0
                # or len(facility.plants.keys()) > 0
            ):
                # combine the plant and animal dictionaries
                arboretum_biome_inhabitants = {
                    **facility.animals,
                ...
                # print the biome name [with id], then join the result of a list comprehension that returns the quantity and name of each inhabitant
                print(
                    f"{facility} ({', '.join(f'{len(arboretum_biome_inhabitants[inhabitant])} {inhabitant}' for inhabitant in arboretum_biome_inhabitants)})"
                )
    plantFacilityTypes = ["plowedFields", "naturalFields"]
    for facilityType in plantFacilityTypes:
        for facility in arboretum.biomes[facilityType]:
            # check if there are entries in the plant or animal dictionaries
            print(facility)
            if (
                len(facility.plants.keys()) > 0
                # or len(facility.plants.keys()) > 0
            ):
                # combine the plant and animal dictionaries
                arboretum_biome_inhabitants = {
